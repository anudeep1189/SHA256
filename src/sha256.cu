...
__global__ void kernel_sha256_hash(BYTE* indata, SHA_WORD inlen, BYTE* outdata, SHA_WORD n_batch)
{
	SHA_WORD thread = blockIdx.x * blockDim.x + threadIdx.x;
	if (thread >= n_batch)
	{
		return;
	}
	BYTE* in = indata; // all threads read the same input
...
cudaError_t mcm_cuda_sha256_hash_batch(BYTE* in, SHA_WORD inlen, BYTE* out, SHA_WORD n_batch)
{
	...
	// Allocate single copy of input on GPU (shared by all threads)
	error = cudaMalloc(&cuda_indata, inlen);
	if (error != cudaSuccess) return error;

	error = cudaMalloc(&cuda_outdata, SHA256_BLOCK_SIZE * n_batch);
	if (error != cudaSuccess) { cudaFree(cuda_indata); return error; }

	cudaMemcpy(cuda_indata, in, inlen, cudaMemcpyHostToDevice);

	SHA_WORD thread = 256;
	SHA_WORD block = (n_batch + thread - 1) / thread;
